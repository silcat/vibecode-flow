...
进入下一阶段前，必须满足当前阶段 README `## 完成证明` 节的退出条件。

- 产出物需落盘的 → `Test-Path` 确认文件存在
- 需执行命令的 → 实际运行，捕获输出，exit code ≠ 0 → 阻断
- 需输出标记的 → 在对话中输出对应标记块，内容不为空
- 需同步 registry 的 → 更新 `docs/work/registry.md` 对应工作项行：流程节点（每阶段出口）、实施节点（实施阶段每 task 完成）
