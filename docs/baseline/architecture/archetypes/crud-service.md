...
模块核心围绕数据实体：增删改查、分页列表、状态流转。选此原型后，以下目录结构、分层规则、代码范例为固定模板，直接套用。`<base-package>` 替换为实际包名。
...
```java
// ✅ 正确
import <base-package>.repository.AaaRepository;
import <base-package>.gateway.BbbGateway;
import <base-package>.domain.CccCalcService;
...
```java
// ❌ 禁止
import <base-package>.bo.YyyBizManageService;           // 调别的 bo
import <base-package>.mapper.XxxMapper;                  // 直接调 mapper
import <base-package>.gateway.feign.XxxFeignClient;     // 直接调 feign
```
...
package <base-package>.gateway.sdk;
...
package <base-package>.gateway.feign;
